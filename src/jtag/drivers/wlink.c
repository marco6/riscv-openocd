...
typedef unsigned long(__stdcall * pCH375SetTimeout) (unsigned long iIndex,
		unsigned long iWriteTimeout, unsigned long iReadTimeout);
typedef unsigned long(__stdcall * pCH375ReadEndP) (unsigned long iIndex,
		unsigned long iPipeNum, void *iBuffer, unsigned long *ioLength);
...
/* FIXME(marco6): original type was jtag_libusb_device_handle */
struct libusb_device_handle *wfd;

int pWriteData(int dev, int endpoint, unsigned char *buf, unsigned long *length)
{
	int ret, pr;
	ret = jtag_libusb_bulk_write(wfd, endpoint, (char *)buf /* FIXME */, *length, 3000, &pr);
	if (ret == ERROR_OK)
		return 1;
	else
		return ret;
...
void wlink_getromram(uint32_t *rom, uint32_t *ram)
{
	...
	if (chip_type == 0x30700508 || chip_type == 0x30710508 || chip_type == 0x30730508
			|| chip_type == 0x30300504 || chip_type == 0x30310504)
		type_A = true;
	if (chip_type == 0x30720508 || chip_type == 0x30740508)
		type_C = true;
	if (chip_type == 0x2034050c || chip_type == 0x2080050c || chip_type == 0x2081050c
			|| chip_type == 0x2082050c || chip_type == 0x2083050c)
		type_B = true;
	switch (rxbuf[3]) {
	case 0:
		if (type_B) {
...
int wlink_init(void)
{
	...
	if (hModule == 0) {
		...
		if (hModule) {
			...
			if (pOpenDev == NULL || pCloseDev == NULL || pSetTimeout == NULL
					|| pReadData == NULL || pWriteData == NULL) {
	...
	if (pReadData(0, 1, rxbuf, &len)) {
		...
		chip_type = (((unsigned int)rxbuf[4])<<24) + (((unsigned int)rxbuf[5])<<16) +
				(((unsigned int)rxbuf[6])<<8) + (((unsigned int) rxbuf[7]));
		...
		case 1:
			...
			if (pReadData(0, 1, rxbuf, &len)) {
				...
				unsigned char Txbuf[52] = { /* FIXME must be a constant */
