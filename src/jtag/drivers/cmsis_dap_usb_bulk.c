...
struct libusb_device_handle *wlink_dev_handle;
...
void wlink_sendchip(uint8_t config)
{
	...
	chip_type = ((unsigned int)buffer_rcode[19]) + (((unsigned int)buffer_rcode[18])<<8)
			+ (((unsigned int)buffer_rcode[17])<<16) + (((unsigned int) buffer_rcode[16])<<24);
...
int wlink_armcheckprotect(void)
{
	...
	if ((*(buffer_rcode + 0) == 0x82) && (*(buffer_rcode + 1) == 0x0c)
			&& (*(buffer_rcode + 2) == 0x01) && (*(buffer_rcode + 3) == 0x01)) {
		libusb_bulk_transfer(wlink_dev_handle, 0x02, buffer_code, sizeof(buffer_code), &transferred, timeout);
		libusb_bulk_transfer(wlink_dev_handle, 0x83, buffer_rcode, sizeof(buffer_rcode), &transferred, timeout);
...
int wlink_armerase(void)
{
	...
	libusb_bulk_transfer(wlink_dev_handle, 0x83, buffer_rcode, sizeof(buffer_rcode), &transferred, timeout);
	if ((*(buffer_rcode + 0) == 0x82) && (*(buffer_rcode + 1) == 0x02)
			&& (*(buffer_rcode + 2) == 0x01) && (*(buffer_rcode + 3) == 0x01))
		return ERROR_OK;
...
int wlink_armwrite(const uint8_t *buffer, uint32_t offset, uint32_t count)
{
	...
	uint8_t address[] = {
		0x81, 0x01, 0x08, *(addr + 3), *(addr + 2), *(addr + 1), *addr,
	...
	while (count > 0) {
		...
		if ((++i % 2) == 0) {
			/* hid_read(wlink_dev_handle, buffer_rcode, 65); */
			libusb_bulk_transfer(wlink_dev_handle, 0x83, buffer_rcode, sizeof(buffer_rcode), &transferred, timeout);
			if (!((*(buffer_rcode + 0) == 0x41) && (*(buffer_rcode + 1) == 0x01) && (*(buffer_rcode + 2) == 0x01)
					&& ((*(buffer_rcode + 3) == 0x02) || (*(buffer_rcode + 3) == 0x04)))) {
