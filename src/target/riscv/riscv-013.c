...
extern unsigned char DMI_OP(unsigned long iIndex, unsigned char iAddr, unsigned long iData,
	unsigned char iOP, unsigned char *oAddr, unsigned long *oData, unsigned char *oOP);
extern unsigned char WriteNonFullPage(unsigned long iaddr, unsigned char *ibuff, unsigned long ilen);
		...
		uint32_t data_out, int timeout_sec, bool exec, bool ensure_success)
{
	int ret = 1;
	if (wchwlink)
		ret = wlink_dmi_op_timeout(target, data_in, dmi_busy_encountered, dmi_op,
				address, data_out, timeout_sec, exec, ensure_success);
	if (ret == ERROR_OK)
		return ret;
	select_dmi(target);
	...
	while (cur_addr < fin_addr) {
		...
		for (unsigned i = start; i < count; ++i) {
			...
			if (setup_needed) {
				...
				/* Turn on autoexec */
				if (!wchwlink) {
					dmi_write(target, DM_ABSTRACTAUTO, 1 << DM_ABSTRACTAUTO_AUTOEXECDATA_OFFSET);
					setup_needed = false;
				}
			} else {
				if (size > 4)
